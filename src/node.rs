...
mod tests {
    ...
    fn test_consensus_seal_verification() {
        ...
        let note_enough_votes = mock_seal(
            0,
            1,
            vec![1],
            &key_pairs[0],
        ...
        assert!(node
            .verify_consensus_seal(&note_enough_votes, vec![0], &mut state)
            .is_err());
    }
    ...
    /// `ViewChange` messages.
    ///
    /// All of these situations will be tested to ensure that they are triggered when (and only
    /// when) expected, and that the targeted view is correct.
    #[test]
    #[allow(unused_must_use)]
    fn test_view_change_initiation_conditions() {
        // TODO: first 3 conditions
